...
exports.build = function(data, boundary, callback) {
  ...
  for (var key in object) {
    ...
    if (value === null || typeof value == 'undefined') {
      ...
      var part = (value.buffer || value.file || value.media || value.content_type) ? value : {value: value};
...
var generate_part = function(name, part, boundary, callback) {
  ...
  if ((part.media || part.file || part.buffer) && part.content_type) {
    var filename = part.filename ? part.filename : part.media ? basename(part.media) : name;
    if (part.buffer) return append(part.buffer, filename);
    readFile(part.media, function(err, data) {
      if (err) return callback(err);
...
var flatten = function(object, into, prefix) {
  ...
  for(var key in object) {
    ...
    if (prop && typeof prop === 'object' && !(prop.buffer || prop.file || prop.media || prop.content_type))
